    ...
    const partner = await database.transaction(async tx => {
      const result = await tx.query(
        `UPDATE partners SET company_name=$2, partner_type='Installer', partner_category=$3, contact_name=$4, email=$5, phone=$6, website=$7, address=$8,
         products_supplied='', installer_share_pct=$9, installer_share_terms=$10, payment_terms=$11, performance_notes=$12, status=$13, updated_at=now()
         WHERE id=$1 AND is_archived=false RETURNING *`,
        [id, company, type, String(body.contact_name || "").trim(), String(body.email || "").trim(), String(body.phone || "").trim(), String(body.website || "").trim(), String(body.address || "").trim(), Number(body.installer_share_pct || 0), String(body.installer_share_terms || "").trim(), String(body.payment_terms || "").trim(), String(body.performance_notes || "").trim(), String(body.status || "Active")],
      );
      ...
      if (Array.isArray(body.project_links)) for (const link of body.project_links) {
        if (link?.project_id) await tx.query("INSERT INTO project_partners (project_id, partner_id, relationship) VALUES ($1,$2,'Installer')", [link.project_id, id]);
      }

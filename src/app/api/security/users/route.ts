...
export async function GET(request: NextRequest) {
  try {
    ...
    const users = await Promise.all(rows.map(async user => { const isAdmin=Boolean(user.is_admin); return { ...user, is_admin:isAdmin, is_active:Boolean(user.is_active), permissions:permissionsForRole(await getUserPermissions(user.id),isAdmin) } }));
...
export async function POST(request: NextRequest) {
  try {
    ...
    await database.transaction(async transaction => {
      await transaction.query("INSERT INTO app_users (id, username, display_name, password_hash, is_admin, is_active) VALUES ($1,$2,$3,$4,$5,true)", [id, username, displayName, createPasswordHash(password), isAdmin ? 1 : 0]);
      ...
      await transaction.query("INSERT INTO audit_logs (entity_type, entity_id, entity_name, action, details) VALUES ('Security', $1, $2, 'Created', $3)", [id, displayName, `${isAdmin ? "Administrator" : "Collaborator"} account created by administrator`]);
    });

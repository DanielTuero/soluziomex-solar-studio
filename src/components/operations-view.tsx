...
import { ArchiveRestore, ChevronLeft, ChevronRight, Clock3, Database, Download, FileClock, HardDriveDownload, RotateCcw, ShieldCheck, Trash2, Upload } from "lucide-react";
...
export function OperationsView() {
  ...
  async function download(backup:DatabaseBackup){setBusy(`download:${backup.name}`);setError("");setMessage("");try{const response=await fetch(`/api/backups/${encodeURIComponent(backup.name)}`,{cache:"no-store"});if(!response.ok){const payload=await response.json();throw new Error(payload.error)}const blob=await response.blob();const picker=(window as SavePickerWindow).showSaveFilePicker;if(picker){const handle=await picker({suggestedName:backup.name,types:[{description:"Solar Studio database backup",accept:{"application/vnd.sqlite3":[".db"]}}]});const writable=await handle.createWritable();await writable.write(blob);await writable.close()}else{const url=URL.createObjectURL(blob);const link=document.createElement("a");link.href=url;link.download=backup.name;link.click();URL.revokeObjectURL(url)}setMessage("Backup saved to your computer.")}catch(reason){if(reason instanceof DOMException&&reason.name==="AbortError")return;setError(reason instanceof Error?reason.message:"Could not download the backup.")}finally{setBusy("")}}
  async function upload(event:ChangeEvent<HTMLInputElement>){const file=event.target.files?.[0];event.target.value="";if(!file)return;setBusy("import");setError("");setMessage("");try{const form=new FormData();form.set("backup",file);const response=await fetch("/api/backups/import",{method:"POST",body:form});const payload=await response.json();if(!response.ok)throw new Error(payload.error);await Promise.all([loadBackups(),loadAudit()]);setMessage("Backup imported and ready to restore.")}catch(reason){setError(reason instanceof Error?reason.message:"Could not import the backup.")}finally{setBusy("")}}
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">Local data protection</p><h1>Backups & audit history</h1><p>Dated database snapshots, portable backup files, safe restore controls, and a chronological record of workspace changes.</p></div><div className="backup-heading-actions"><input ref={importInput} type="file" accept=".db,.sqlite,.sqlite3,application/vnd.sqlite3,application/octet-stream" onChange={upload} hidden/><button className="button secondary" onClick={()=>importInput.current?.click()} disabled={Boolean(busy)}><Upload size={16}/>{busy==="import"?"Importing…":"Import backup"}</button><button className="button primary" onClick={create} disabled={Boolean(busy)}><HardDriveDownload size={16}/>{busy==="create"?"Creating…":"Back up database"}</button></div></div>
    ...
    <div className="operations-grid">
      <section className="card backup-card"><div className="card-header"><div><h2>Database snapshots</h2><p>One automatic backup is created each day when Solar Studio opens.</p></div><span className="backup-count"><Database size={14}/>{backups.length}</span></div>{backups.length?<div className="backup-list">{backups.map(backup=><article key={backup.name}><span className={`backup-icon ${backup.kind}`}><ArchiveRestore size={17}/></span><div><strong>{backup.kind} backup</strong><span>{dateTime(backup.created_at)} · {size(backup.size)}</span><small>{backup.name}</small></div><div className="backup-actions"><button className="button ghost small" onClick={()=>download(backup)} disabled={Boolean(busy)} title="Save backup to your computer"><Download size={12}/>{busy===`download:${backup.name}`?"Saving…":"Download"}</button><button className="button secondary small" onClick={()=>restore(backup)} disabled={Boolean(busy)}><RotateCcw size={12}/>{busy===backup.name?"Working…":"Restore"}</button></div><button className="button ghost small" onClick={()=>remove(backup)} disabled={Boolean(busy)} title="Delete backup"><Trash2 size={13}/></button></article>)}</div>:<div className="empty-state"><Database size={30}/><h3>No backups yet</h3><p>Create or import the first snapshot of the local database.</p></div>}<div className="backup-safety"><ShieldCheck size={16}/><div><strong>Restore safety</strong><span>Imported files are checked before they are saved. Before any restore, Solar Studio automatically preserves the current database as a new manual backup.</span></div></div></section>
      <aside className="card backup-info"><Clock3 size={20}/><h3>Automatic protection</h3><p>Daily snapshots are complete copies of projects, catalogs, economics, images, partner records, security settings, and history.</p><div><strong>Portable copies</strong><span>Download a snapshot to any folder, then import the .db file later to restore that version.</span></div></aside>
    </div>

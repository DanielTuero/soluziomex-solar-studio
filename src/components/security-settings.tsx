...
export function SecuritySettings() {
  ...
  async function loadUsers(){const response=await fetch("/api/security/users",{cache:"no-store"});const payload=await response.json();if(!response.ok)throw new Error(payload.error);setUsers(payload.users)}
  useEffect(()=>{fetch("/api/security/status",{cache:"no-store"}).then(response=>response.json()).then(async state=>{setEnabled(Boolean(state.enabled));setIsAdmin(Boolean(state.user?.is_admin));if(state.user?.is_admin)await loadUsers()}).catch(reason=>setError(reason.message)).finally(()=>setLoading(false))},[]);

  async function toggleProtection(){setLoading(true);clearNotices();const next=!enabled;try{const response=await fetch("/api/security/settings",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:next})});const result=await response.json();if(!response.ok)throw new Error(result.error);setEnabled(result.enabled);setMessage(result.enabled?"Password entry is enabled.":"Password entry is disabled for this computer.")}catch(reason){setError(reason instanceof Error?reason.message:"Could not update this setting.")}finally{setLoading(false)}}
  async function changePassword(event:FormEvent<HTMLFormElement>){event.preventDefault();clearNotices();setBusy("password");const form=new FormData(event.currentTarget);const next=String(form.get("new_password")??"");if(next!==String(form.get("confirm_password")??"")){setError("The new passwords do not match.");setBusy("");return}try{const response=await fetch("/api/security/password",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({current_password:form.get("current_password"),new_password:next})});const result=await response.json();if(!response.ok)throw new Error(result.error);event.currentTarget.reset();setEnabled(true);setMessage("Your password was changed. Other signed-in sessions were closed.")}catch(reason){setError(reason instanceof Error?reason.message:"Could not change the password.")}finally{setBusy("")}}
  async function createUser(event:FormEvent<HTMLFormElement>){event.preventDefault();clearNotices();setBusy("create-user");const form=new FormData(event.currentTarget);try{const response=await fetch("/api/security/users",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({display_name:form.get("display_name"),username:form.get("username"),password:form.get("password"),permissions:newPermissions})});const result=await response.json();if(!response.ok)throw new Error(result.error);event.currentTarget.reset();setNewPermissions(["portfolio","projects"]);await loadUsers();setMessage("New local user created.")}catch(reason){setError(reason instanceof Error?reason.message:"Could not create the user.")}finally{setBusy("")}}
  async function saveUser(user:ManagedUser,password:string){clearNotices();setBusy(user.id);try{const response=await fetch(`/api/security/users/${encodeURIComponent(user.id)}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({...user,password})});const result=await response.json();if(!response.ok)throw new Error(result.error);await loadUsers();setMessage(`${user.display_name}'s access was updated.`)}catch(reason){setError(reason instanceof Error?reason.message:"Could not update the user.")}finally{setBusy("")}}
  async function removeUser(user:ManagedUser){if(!confirm(`Delete the local account for ${user.display_name}?`))return;clearNotices();setBusy(user.id);try{const response=await fetch(`/api/security/users/${encodeURIComponent(user.id)}`,{method:"DELETE"});if(!response.ok){const result=await response.json();throw new Error(result.error)}await loadUsers();setMessage("Local user deleted.")}catch(reason){setError(reason instanceof Error?reason.message:"Could not delete the user.")}finally{setBusy("")}}

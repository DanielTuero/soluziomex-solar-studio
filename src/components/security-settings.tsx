...
export function SecuritySettings() {
  ...
  const [newRole,setNewRole]=useState<"collaborator"|"admin">("collaborator");
  const [newPermissions,setNewPermissions]=useState<string[]>(["portfolio","projects","security"]);
  ...
  async function createUser(event:FormEvent<HTMLFormElement>){event.preventDefault();clearNotices();setBusy("create-user");const formElement=event.currentTarget;const form=new FormData(formElement);try{const response=await fetch("/api/security/users",{method:"POST",credentials:"same-origin",headers:securityHeaders(securityToken,true),body:JSON.stringify({display_name:form.get("display_name"),username:form.get("username"),password:form.get("password"),is_admin:newRole==="admin",permissions:newPermissions})});const result=await response.json();if(!response.ok)throw new Error(result.error);formElement.reset();setNewRole("collaborator");setNewPermissions(["portfolio","projects","security"]);await loadUsers();setMessage(`New ${newRole} created.`)}catch(reason){setError(reason instanceof Error?reason.message:"Could not create the user.")}finally{setBusy("")}}
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">Workspace settings</p><h1>{isAdmin?"Security & users":"Security"}</h1><p>{isAdmin?"Manage local passwords, roles, and which parts of Solar Studio each person can access.":"Change the password for your Solar Studio account."}</p></div></div>
    ...
    {isAdmin&&<>
      ...
      <section className="card create-user-card">
        <div className="security-heading"><span><UserPlus size={22}/></span><div><h2>Create a new user</h2><p>Choose an administrator with full control or a collaborator with limited menu access.</p></div></div>
        <form onSubmit={createUser}>
          <div className="security-user-fields"><label><span>Display name</span><input name="display_name" placeholder="María López" required/></label><label><span>Username</span><input name="username" placeholder="maria" autoComplete="off" required/></label><label><span>Starting password</span><input name="password" type="password" minLength={4} autoComplete="new-password" required/></label><label><span>Role</span><select value={newRole} onChange={event=>setNewRole(event.target.value as "collaborator"|"admin")}><option value="collaborator">Collaborator</option><option value="admin">Administrator</option></select></label></div>
          {newRole==="admin"?<div className="admin-full-access"><ShieldCheck size={15}/>Administrators can manage users and access every Solar Studio section.</div>:<PermissionSwitches permissions={newPermissions} toggle={section=>section!=="security"&&setNewPermissions(current=>current.includes(section)?current.filter(item=>item!==section):[...current,section])}/>}
...
function UserAccessCard({user,busy,update,togglePermission,save,remove}:{user:ManagedUser;busy:boolean;update:(changes:Partial<ManagedUser>)=>void;togglePermission:(section:string)=>void;save:(password:string)=>void;remove:()=>void}){
  ...
  return <article className={`user-access-card ${!user.is_active?"inactive":""}`}><div className="user-access-head"><div className="user-avatar">{user.display_name.split(/\s+/).map(part=>part[0]).join("").slice(0,2).toUpperCase()}</div><div><input aria-label="Display name" value={user.display_name} onChange={event=>update({display_name:event.target.value})} disabled={isMainAdmin}/><span>@{user.username} · {user.is_admin?"Administrator":"Collaborator"}{isMainAdmin?" · Main account":""}</span></div>{!isMainAdmin&&<div className="user-access-controls"><label className="user-role"><span>Role</span><select value={user.is_admin?"admin":"collaborator"} onChange={event=>update({is_admin:event.target.value==="admin"})}><option value="collaborator">Collaborator</option><option value="admin">Administrator</option></select></label><label className="account-active"><span>Active</span><button type="button" className={`switch mini ${user.is_active?"on":""}`} onClick={()=>update({is_active:!user.is_active})} role="switch" aria-checked={user.is_active}><span/></button></label></div>}</div>
    {user.is_admin?<div className="admin-full-access"><ShieldCheck size={15}/>Full access to every Solar Studio section and user management</div>:<PermissionSwitches permissions={user.permissions} toggle={section=>section!=="security"&&togglePermission(section)}/>}
    {!isMainAdmin&&<div className="user-card-actions"><label><span>Reset password (optional)</span><input type="password" value={password} onChange={event=>setPassword(event.target.value)} placeholder="Leave blank to keep it" minLength={4}/></label><button type="button" className="button secondary small" onClick={()=>{save(password);setPassword("")}} disabled={busy}><Save size={12}/>{busy?"Saving…":"Save user"}</button><button type="button" className="button ghost small danger-text" onClick={remove} disabled={busy} title="Delete user"><Trash2 size={13}/></button></div>}
  </article>;
}

function PermissionSwitches({permissions,toggle}:{permissions:string[];toggle:(section:string)=>void}){return <div className="permission-switches">{sections.map(section=><div key={section.id}><span>{section.label}{section.id==="security"?" (password)":""}</span><button type="button" className={`switch mini ${permissions.includes(section.id)?"on":""}`} onClick={()=>toggle(section.id)} role="switch" aria-checked={permissions.includes(section.id)} disabled={section.id==="security"} title={section.id==="security"?"Collaborators always have access to their password settings":undefined}><span/></button></div>)}</div>}

...
type ProjectLinkDraft = { relationship: string; installer_share_pct: number; installer_share_terms: string };
...
export function PartnersView() {
  ...
  async function savePartner(event: FormEvent<HTMLFormElement>, links: Array<{project_id:string;relationship:string;installer_share_pct:number;installer_share_terms:string}>) {
...
function PartnerCard({partner,edit,remove,addQuote,editQuote,removeQuote}:{partner:Partner;edit:()=>void;remove:()=>void;addQuote:()=>void;editQuote:(quote:PartnerQuote)=>void;removeQuote:(quote:PartnerQuote)=>void}) {
  return <article className="card partner-card">
    ...
    <div className="partner-details"><div><span>Payment terms</span><strong>{partner.payment_terms||"Not specified"}</strong></div><div><span>Performance notes</span><strong>{partner.performance_notes||"No performance notes yet."}</strong></div></div>
    <div className="partner-projects"><span>Active projects</span>{partner.projects.length?<div>{partner.projects.map(project=><i key={`${project.id}-${project.relationship}`}>{project.code} · {project.name}<small>{project.relationship}{partner.partner_type==="Installer"?` · ${project.installer_share_pct}% share`:""}{project.installer_share_terms?` · ${project.installer_share_terms}`:""}</small></i>)}</div>:<p>No linked active projects.</p>}</div>
...
function PartnerModal({partner,projects,close,submit}:{partner:Partner|null;projects:ProjectOption[];close:()=>void;submit:(event:FormEvent<HTMLFormElement>,links:Array<{project_id:string;relationship:string;installer_share_pct:number;installer_share_terms:string}>)=>void}) {
  const [type,setType]=useState<PartnerType>(partner?.partner_type||"Installer");
  const initial=Object.fromEntries((partner?.projects||[]).map(project=>[project.id,{
    relationship:project.relationship,
    installer_share_pct:Number(project.installer_share_pct||0),
    installer_share_terms:project.installer_share_terms||"",
  }]));
  const [links,setLinks]=useState<Record<string,ProjectLinkDraft>>(initial);
  const toggle=(id:string,checked:boolean)=>setLinks(current=>{const next={...current};if(checked)next[id]={relationship:type,installer_share_pct:0,installer_share_terms:""};else delete next[id];return next});
  const updateLink=(id:string,values:Partial<ProjectLinkDraft>)=>setLinks(current=>({...current,[id]:{...current[id],...values}}));
  const submittedLinks=Object.entries(links).map(([project_id,link])=>({project_id,...link,relationship:type}));
  return <div className="modal-backdrop"><form className="modal partner-modal" onSubmit={event=>submit(event,submittedLinks)}><div className="modal-head"><div><p className="eyebrow">Contact directory</p><h2>{partner?"Edit contact":"Add project contact"}</h2></div><button type="button" className="button ghost small" onClick={close}>×</button></div><div className="modal-body"><div className="form-grid">
    <Field name="company_name" label="Company or office name" defaultValue={partner?.company_name} required/>
    <div className="form-field"><label>Contact category</label><select name="partner_type" value={type} onChange={event=>setType(event.target.value as PartnerType)}>{partnerCategories.map(category=><option key={category}>{category}</option>)}</select></div>
    <Field name="contact_name" label="Contact person" defaultValue={partner?.contact_name}/><Field name="email" label="Email" type="email" defaultValue={partner?.email}/><Field name="phone" label="Phone" defaultValue={partner?.phone}/><Field name="website" label="Website" defaultValue={partner?.website}/>
    <div className="form-field full"><label>Address</label><input name="address" defaultValue={partner?.address}/></div>
    <Field name="payment_terms" label="Payment terms" defaultValue={partner?.payment_terms} placeholder="Net 30, paid after completion…"/>
    <div className="form-field full"><label>Performance notes</label><textarea name="performance_notes" defaultValue={partner?.performance_notes} placeholder="Responsiveness, technical ability, reliability, useful context…"/></div>
    <div className="form-field"><label>Status</label><select name="status" defaultValue={partner?.status||"Active"}><option>Active</option><option>Inactive</option></select></div>
    <div className="form-field full"><label>Active projects</label><span className="form-help">{type==="Installer"?"Set the agreed revenue share separately for every project assignment.":"Choose the projects connected to this contact."}</span><div className="project-link-list">{projects.map(project=>{const link=links[project.id];return <div className={link?"project-link-entry linked":"project-link-entry"} key={project.id}>
      <label className="project-link-check"><input type="checkbox" checked={Boolean(link)} onChange={event=>toggle(project.id,event.target.checked)}/><span><strong>{project.code} · {project.name}</strong><small>{project.status}</small></span></label>
      {link&&type==="Installer"&&<div className="project-share-fields"><label><span>Revenue share</span><span className="share-input"><input aria-label={`Revenue share for ${project.name}`} type="number" min="0" max="100" step="0.1" value={link.installer_share_pct} onChange={event=>updateLink(project.id,{installer_share_pct:Math.max(0,Number(event.target.value))})}/><b>%</b></span></label><label className="share-terms"><span>Project-specific terms</span><input aria-label={`Revenue-share terms for ${project.name}`} value={link.installer_share_terms} onChange={event=>updateLink(project.id,{installer_share_terms:event.target.value})} placeholder="Paid monthly after collection…"/></label></div>}
    </div>})}</div></div>
  </div></div><div className="modal-actions"><button type="button" className="button secondary" onClick={close}>Cancel</button><button className="button primary">{partner?"Save changes":"Add contact"}</button></div></form></div>;
}

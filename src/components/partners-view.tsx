...
import { Building2, CircleDollarSign, FileText, Handshake, Mail, MapPin, Pencil, Phone, Plus, Search, Trash2, Users } from "lucide-react";
...
export function PartnersView() {
  ...
  const visible = useMemo(() => partners.filter(partner => {
    const matchesType = type === "All" || partner.partner_type === type || partner.partner_type === "Both";
    const text = `${partner.company_name} ${partner.contact_name} ${partner.products_supplied}`.toLowerCase();
    return matchesType && text.includes(query.toLowerCase());
  }), [partners, query, type]);
  const totalQuotes = partners.reduce((sum, partner) => sum + partner.quotes.length, 0);
  const activeProjects = new Set(partners.flatMap(partner => partner.projects.map(project => project.id))).size;
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">Commercial partner network</p><h1>Suppliers & installers</h1><p>Contacts, terms, project relationships, performance notes, and quote history in one reusable directory.</p></div><button className="button primary" onClick={() => setEditing("new")}><Plus size={16}/>Add partner</button></div>
    <div className="metric-grid partner-metrics"><Metric icon={Building2} label="Directory companies" value={partners.length}/><Metric icon={Handshake} label="Installers" value={partners.filter(p=>p.partner_type!=="Supplier").length}/><Metric icon={FileText} label="Quotes recorded" value={totalQuotes}/><Metric icon={Users} label="Active linked projects" value={activeProjects}/></div>
    {error && <div className="error-banner">{error}</div>}
    <div className="partner-toolbar"><div className="filters"><div className="partner-search"><Search size={14}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search company, contact, products…"/></div><select className="inline-select" value={type} onChange={event=>setType(event.target.value)}><option>All</option><option>Supplier</option><option>Installer</option></select></div><span>{visible.length} partner{visible.length===1?"":"s"}</span></div>
...
function Metric({icon:Icon,label,value}:{icon:React.ElementType;label:string;value:number}) { return <div className="metric-card"><div className="metric-top"><span>{label}</span><span className="metric-icon"><Icon size={15}/></span></div><div className="metric-value">{value}</div><div className="metric-note">Across the local workspace</div></div>; }
...
function PartnerCard({partner,edit,remove,addQuote,editQuote,removeQuote}:{partner:Partner;edit:()=>void;remove:()=>void;addQuote:()=>void;editQuote:(quote:PartnerQuote)=>void;removeQuote:(quote:PartnerQuote)=>void}) {
  return <article className="card partner-card">
    ...
    <div className="partner-details"><div><span>Products supplied</span><strong>{partner.products_supplied||"Not specified"}</strong></div><div><span>Payment terms</span><strong>{partner.payment_terms||"Not specified"}</strong></div>{partner.partner_type!=="Supplier"&&<div><span>Installer revenue-share terms</span><strong>{partner.installer_share_pct}% of customer revenue{partner.installer_share_terms?` · ${partner.installer_share_terms}`:""}</strong></div>}<div><span>Performance notes</span><strong>{partner.performance_notes||"No performance notes yet."}</strong></div></div>
...
function PartnerModal({partner,projects,close,submit}:{partner:Partner|null;projects:ProjectOption[];close:()=>void;submit:(event:FormEvent<HTMLFormElement>,links:Array<{project_id:string;relationship:string}>)=>void}) {
  const [type,setType]=useState<PartnerType>(partner?.partner_type||"Supplier");
  ...
  const toggle=(id:string,checked:boolean)=>setLinks(current=>{const next={...current};if(checked)next[id]=type==="Installer"?"Installer":"Supplier";else delete next[id];return next});
  return <div className="modal-backdrop"><form className="modal partner-modal" onSubmit={event=>submit(event,Object.entries(links).map(([project_id,relationship])=>({project_id,relationship})))}><div className="modal-head"><div><p className="eyebrow">Partner directory</p><h2>{partner?"Edit partner":"Add supplier or installer"}</h2></div><button type="button" className="button ghost small" onClick={close}>×</button></div><div className="modal-body"><div className="form-grid"><Field name="company_name" label="Company name" defaultValue={partner?.company_name} required/><div className="form-field"><label>Partner type</label><select name="partner_type" value={type} onChange={event=>setType(event.target.value as PartnerType)}><option>Supplier</option><option>Installer</option><option>Both</option></select></div><Field name="contact_name" label="Contact person" defaultValue={partner?.contact_name}/><Field name="email" label="Email" type="email" defaultValue={partner?.email}/><Field name="phone" label="Phone" defaultValue={partner?.phone}/><Field name="website" label="Website" defaultValue={partner?.website}/><div className="form-field full"><label>Address</label><input name="address" defaultValue={partner?.address}/></div><div className="form-field full"><label>Products supplied</label><textarea name="products_supplied" defaultValue={partner?.products_supplied} placeholder="Solar panels, inverters, cable, racking…"/></div>{type!=="Supplier"&&<><Field name="installer_share_pct" label="Installer revenue share (%)" type="number" min="0" max="100" step="0.1" defaultValue={partner?.installer_share_pct||0}/><Field name="installer_share_terms" label="Revenue-share terms" defaultValue={partner?.installer_share_terms} placeholder="Paid monthly after collection; 15-year term…"/></>}<Field name="payment_terms" label="Payment terms" defaultValue={partner?.payment_terms} placeholder="50% order / 50% delivery, Net 30…"/><div className="form-field full"><label>Performance notes</label><textarea name="performance_notes" defaultValue={partner?.performance_notes} placeholder="Quality, responsiveness, delivery reliability, warranty support…"/></div><div className="form-field"><label>Status</label><select name="status" defaultValue={partner?.status||"Active"}><option>Active</option><option>Inactive</option></select></div><div className="form-field full"><label>Active projects</label><div className="project-link-list">{projects.map(project=><div key={project.id}><label><input type="checkbox" checked={Boolean(links[project.id])} onChange={event=>toggle(project.id,event.target.checked)}/><span><strong>{project.code} · {project.name}</strong><small>{project.status}</small></span></label>{links[project.id]&&type==="Both"&&<select value={links[project.id]} onChange={event=>setLinks(current=>({...current,[project.id]:event.target.value}))}><option>Supplier</option><option>Installer</option></select>}</div>)}</div></div></div></div><div className="modal-actions"><button type="button" className="button secondary" onClick={close}>Cancel</button><button className="button primary">{partner?"Save changes":"Add partner"}</button></div></form></div>;
}
